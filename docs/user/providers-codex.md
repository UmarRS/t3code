...
This guide is for people who want to use more than one Codex account in Atlas. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Atlas](./install.md).
...
In Atlas Settings, name it something obvious:
...
In Atlas Settings, add another Codex provider:
...
Atlas shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Atlas treats it
