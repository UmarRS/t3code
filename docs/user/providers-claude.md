...
This guide is for people who want to use more than one Claude setup in Atlas. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Atlas](./install.md).
...
In Atlas Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Atlas uses Claude Code's normal config directory.

When you set this field, Atlas points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Atlas looks.

Then add another Claude provider in Atlas:
...
Atlas only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Atlas keeps separate config directories isolated
...
Add or edit a Claude provider in Atlas Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Atlas stores the value as a server secret and does not
...
Atlas does not need a special Claude Code Router provider. Treat the router as a Claude
