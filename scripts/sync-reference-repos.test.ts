...
import { type ReferenceRepo, referenceRepos } from "./lib/reference-repos.ts";
import {
...
// A JSON-sourced repo definition, so the version-source error paths stay
// covered independently of which repos this build actually vendors.
const jsonSourcedRepo: ReferenceRepo = {
  id: "json-sourced",
  prefix: ".repos/json-sourced",
  repository: "https://github.com/example/json-sourced.git",
  latestRef: "main",
...
it.layer(NodeServices.layer)("sync-reference-repos", (it) => {
  ...
  it.effect("preserves version source parse context and the schema cause", () =>
    Effect.gen(function* () {
      ...
      const sourcePath = path.join(rootDir, jsonSourcedRepo.versionSourcePath);
      yield* fs.makeDirectory(path.dirname(sourcePath), { recursive: true });
      yield* fs.writeFileString(sourcePath, "{");

      const error = yield* resolveReferenceRepoRef(jsonSourcedRepo, rootDir, false).pipe(
        Effect.flip,
      );
      ...
      assert.equal(error.repoId, jsonSourcedRepo.id);
      assert.equal(error.sourcePath, sourcePath);
      assert.ok(error.cause !== undefined);
      assert.ok(!error.message.includes(String((error.cause as Error).message)));
    }),
