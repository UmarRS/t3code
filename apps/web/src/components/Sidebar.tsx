...
export default function Sidebar() {
  ...
  // Settled work is project history, not inbox work. Keep the globally
  // recency-sorted rows in their logical project buckets; the project rows
  // own expansion and paging so collapsed history costs no DOM work.
  const settledThreadsByProjectKey = useMemo(() => {
    const groups = groupSettledThreadsByProject(
      settledThreads,
      (projectKey) => settledProjectIdentityByKey.get(projectKey) ?? null,
    );
    return new Map(groups.map((group) => [group.projectKey, group.threads] as const));
  }, [settledProjectIdentityByKey, settledThreads]);
  ...
  const orderedThreads = useMemo(
    () => [...pinnedThreads, ...activeThreads, ...visibleSnoozedThreads],
    [pinnedThreads, activeThreads, visibleSnoozedThreads],
  );
  ...
  const threadByKey = useMemo(
    () =>
      new Map(
        [...orderedThreads, ...settledThreads].map(
          (thread) =>
            [scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)), thread] as const,
        ),
      ),
    [orderedThreads, settledThreads],
  );
  ...
  const renderThreadRow = (
    ...
    // other thread is a full card. Settled rows are further recessed inside
    // their project, leaving the top-level list as the work inbox.
    ...
    return (
      <SidebarThreadRow
        ...
        jumpLabel={
          section !== "settled" && showJumpHints ? (jumpLabelByKey.get(threadKey) ?? null) : null
        }
    ...
    <>
      ...
      >
        <SidebarGroup className="ps-[calc(var(--sidebar-content-inset)+1px)] pe-[var(--sidebar-content-inset)] pb-1 pt-0">
          ...
          pinnedThreads.length + activeThreads.length + snoozedThreads.length === 0 ? (
            <div className="flex flex-col items-center gap-2 px-2 py-6 text-center text-xs text-muted-foreground/60">
              {projects.length === 0 ? (
                ...
                `No unsettled work in ${scopedProjectGroup.displayName}`
              ) : (
                "No unsettled work"
              )}
            </div>
