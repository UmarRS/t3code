// Pool registry for multiple backend processes.
//
//   - `DesktopBackendManager.ts` exposes a per-instance factory
//     (`makeBackendInstance(spec)`); the pool calls it once for the primary
//     at startup. Additional instances can be brought up through
//     `pool.register`.
//   - The primary spec wires `configResolve` to
//     `DesktopBackendConfiguration.resolvePrimary` and the
//     `onReady`/`onShutdown` callbacks to the window service. Secondary
//     instances skip onReady/onShutdown — the window only follows the primary.
//   - The pool exposes `register(spec)` and `unregister(id)`. Each registered
//     instance gets its own child scope, so unregister can stop it cleanly
//     without tearing down the pool. The primary's id refuses unregister.
//   - `getLocalEnvironmentBootstraps()` (plural) returns one entry per pool
//     instance currently registered with bootstrap info. The primary keeps the
//     "primary" id.
//
// Renderer-side wiring: secondary instances are reconciled into the
// connection catalog as desktop-local saved environments — the renderer POSTs
// the shared bootstrap token to /api/auth/bootstrap/bearer on that backend's
// URL, fetches the descriptor, and opens a connection through the same
// saved-env path remote envs use. The desktopLocal marker filters those
// records out of saved-env persistence.
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // calls, so on app shutdown a backend child process gets hard-killed
    // by the OS instead of receiving the graceful SIGTERM + grace period
    // the instance's stop finalizer would otherwise run.
